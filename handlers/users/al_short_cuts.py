...
async def pros(message: types.Message):
    await message.answer('Tanlang', reply_markup=menu)
    await ShopState.category.set()
...
async def a_menu(message: types.Message):
...
# @dp.message_handler(state=ShopState.check, text='Tasdiqlash')
# async def check(state: FSMContext, message: types.Message):
